 ...
 *  'return-skipped', 'return-leg', 'relay-return', 'intent-yes'/'intent-no', 'more-like-this').
 *  'relay-return' = the relay delivered the return leg (lib/relay) — it precedes the
 *  'return-leg' the confirm page then fires, so the two together split automatic round-trips
 *  from manual pastes. 'return-skipped' = the return gate was dismissed without sending (the
 *  measurable drop-off). Fire-and-forget; one retry if count.js is still loading. */
